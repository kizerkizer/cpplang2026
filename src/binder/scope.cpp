...
void Scope::setMyNode(Node* node) {
    m_node = node;
}

Node* Scope::getMyNode() const {
    return m_node;
}
...
Symbol* Scope::getSymbol(const std::string& nameString) const {
    ...
    if (name) {
        ...
        return m_parent->getSymbol(nameString);
    }
    return nullptr;
}

Symbol* Scope::getSymbolShallow(const std::string& nameString) const {
    auto it = m_symbols.find(nameString);
    if (it != m_symbols.end()) {
...
bool Scope::putSymbol(std::unique_ptr<Symbol> name) {
    auto nameString = name->getNameString();
    if (this->hasSymbolShallow(nameString)) {
        return false;
    }
    m_symbols[nameString] = std::move(name);
    return true;
}

Scope* Scope::getFirstFunctionContainingScope() {
    if (m_kind == ScopeKind::Function) {
        return this;
    }
    if (m_parent) {
        return m_parent->getFirstFunctionContainingScope();
    }
    return nullptr;
}

Scope* Scope::getFirstLoopContainingScope() {
    if (m_kind == ScopeKind::Loop) {
        return this;
    }
    if (m_parent) {
        return m_parent->getFirstLoopContainingScope();
    }
    return nullptr;
}

Scope* Scope::getThisKeywordScope() const {
    if (m_thisKeywordScope) {
        return m_thisKeywordScope;
    }
    if (m_parent) {
        return m_parent->getThisKeywordScope();
    }
    return nullptr;
}

void Scope::setThisKeywordScope(Scope* thisKeywordScope) {
    m_thisKeywordScope = thisKeywordScope;
}